...
void erosion(int total_times,int size, Mat On, Mat otpt)
{
	int erosion_type;//Erosion and Dilation initial declaration
	erosion_type = MORPH_RECT;//Erosion type
	int erosion_size=size; //size of the kernel will be erosion_size*2+1
	Mat element =  getStructuringElement( erosion_type,
		Size( 2*erosion_size + 1, 2*erosion_size+1 ),
		Point( erosion_size, erosion_size ) );

	for (int i = 0; i < total_times; i++)
		erode(On,otpt, element );
}

void dilation(int total_times,int size, Mat On, Mat otpt)
{
	int erosion_type;//Erosion and Dilation initial declaration
	erosion_type = MORPH_RECT;//Erosion type
	int erosion_size=size; //size of the kernel will be erosion_size*2+1
...
int main(int argc, char** argv)
{
	...
	if (a=='i') //If image
	{
		...
		while(1)
		{
			...
			erosion(1,1,required,required);
			dilation(1,4,required,required);
	...
	if(a=='v')//If video
	{
		...
		while(1)
		{
			...
			erosion(1,2,required,required);
			GaussianBlur( required, required, Size(3, 3), 2, 2 );//Applying Gaussian Blur of parameter 1,1
			GaussianBlur( required, required, Size(3, 3), 2, 2 );//Applying Gaussian Blur of parameter 1,1
			GaussianBlur( required, required, Size(3, 3), 2, 2 );//Applying Gaussian Blur of parameter 1,1
			// dilation(1,2,required,required);
